// handlers/webinfo.js â€” WEB & INFO + API PUBLIK gratis + FREEINFO
...
async function handleWebinfo(ctx) {
  const { sock, m, jid, isGroup, sender, body, cmd, args, prefix, start, unwrapped } = ctx;
    ...
    if (cmd === 'search') {
      ...
      await interim(sock, jid, m, 'ðŸ”Ž Lagi cari di web...');
      // Tahap 1: Wikipedia
      try {
        const w = await wikiSummary(args);
        const out = `ðŸ“– *${w.title}*\n${w.extract}${w.url ? `\nðŸ”— ${w.url}` : ''}`;
        pushMessage(jid, 'user', '.search ' + args);
        pushMessage(jid, 'bot', out.slice(0, 2000));
        await safeReply(sock, jid, `ðŸ”Ž *Hasil "${args}" (Wikipedia):*\n\n${out}`, m); return true;
      } catch (e) {
      ...
      try {
        ...
        console.error('search', e?.message || e);
        await safeReply(sock, jid, 'âŒ Pencarian gagal total. Coba kata kunci lain ya.', m); return true;
      }
    }
    if (cmd === 'news' || cmd === 'berita') {
      ...
      await interim(sock, jid, m, 'ðŸ“° Lagi ambil berita...');
      try {
        const list = await googleNews(args);
        if (list.length === 0) throw new Error('kosong');
        const out = list
          .map((n, i) => `${i + 1}. *${n.title}*\n   ðŸ—“ï¸ ${n.pubDate}${n.link ? `\n   ðŸ”— ${n.link}` : ''}`)
          .join('\n\n');
        pushMessage(jid, 'user', '.news ' + args);
        pushMessage(jid, 'bot', out.slice(0, 2000));
        await safeReply(sock, jid, `ðŸ“° *Berita "${args}":*\n\n${out}`, m); return true;
      } catch (e) {
        console.error('news', e?.message || e);
        await safeReply(sock, jid, 'âŒ Gagal ambil berita. Coba topik lain / ulangi sebentar lagi.', m); return true;
      }
    }
    if (cmd === 'weather' || cmd === 'cuaca') {
      ...
      try {
        ...
        await safeReply(
          sock, jid,
          `ðŸŒ¤ï¸ *Cuaca ${w.name}${w.country ? ', ' + w.country : ''}*\n` +
          `Kondisi: ${w.desc}\nðŸŒ¡ï¸ Suhu: ${w.temp}Â°C (min ${w.min}Â°C / max ${w.max}Â°C)\nðŸ’§ Kelembapan: ${w.humidity}%\nðŸ’¨ Angin: ${w.wind} km/jam`,
          m
        ); return true;
      } catch (e) {
        ...
        if (msg.includes('tidak ditemukan')) {
          await safeReply(sock, jid, `âŒ Kota "${args}" tidak ditemukan. Coba nama kota lain ya.`, m); return true;
        }
        await safeReply(sock, jid, 'âŒ Gagal ambil cuaca. Coba lagi sebentar ya.', m); return true;
      }
    }
    if (cmd === 'time' || cmd === 'jam') {
      if (!args) {
      ...
      try {
        ...
        await safeReply(sock, jid, `ðŸ•’ *Waktu di ${args}*\n${str}\n(${t.zone}${t.abbrev ? ' â€¢ ' + t.abbrev : ''})`, m); return true;
      } catch (e) {
        console.error('time', e?.message || e);
        await safeReply(sock, jid, 'âŒ Gagal ambil waktu. Coba lagi sebentar ya.', m); return true;
      }
    }
    ...
    if (cmd === 'aio' || cmd === 'spotify' || cmd === 'gdrive' || cmd === 'deepsearch') {
      ...
      const labels = { aio: 'â¬‡ï¸', spotify: 'ðŸŽµ', gdrive: 'ðŸ“', deepsearch: 'ðŸ”Ž' };
      await interim(sock, jid, m, `${labels[cmd]} Memproses lewat API publik gratis...`);
      try {
        if (cmd === 'deepsearch') {
          const result = await publicApi.deepSearch(args);
          await sendLongText(sock, jid, `ðŸ”Ž *Deep Search*\n\n${String(result || 'Tidak ada hasil.')}`, m); return true;
        }
        if (cmd === 'spotify') {
          const result = await publicApi.spotify(args);
          await sock.sendMessage(jid, { audio: { url: result.url }, mimetype: 'audio/mpeg', caption: `ðŸŽµ ${result.title || 'Spotify'}${result.artist ? ` â€” ${result.artist}` : ''}` }, { quoted: m }); return true;
        }
        if (cmd === 'gdrive') {
          const result = await publicApi.googleDrive(args);
          await sock.sendMessage(jid, { document: { url: result.url }, fileName: result.name || 'google-drive-file', mimetype: 'application/octet-stream', caption: `ðŸ“ ${result.name || 'File Google Drive'}` }, { quoted: m }); return true;
        }
        const result = await publicApi.aio(args);
        const media = result.media;
        const caption = result.title ? `ðŸŽ¬ ${result.title}` : 'ðŸŽ¬ Hasil download';
        ...
        await safeReply(sock, jid, `âŒ ${e?.message || 'API publik sedang tidak tersedia.'}`, m); return true;
      }
    }
