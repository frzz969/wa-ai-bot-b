// handlers/jere-util.js â€” JERE-UTIL
...
async function handleJereUtil(ctx) {
  const { sock, m, jid, isGroup, sender, body, cmd, args, prefix, start, unwrapped } = ctx;
    ...
    if (cmd === 'jyts') {
      ...
      await interim(sock, jid, m, 'ðŸ” Lagi search YouTube (Jere)...');
      try {
    ...
    if (cmd === 'jspotify') {
      ...
      await interim(sock, jid, m, 'ðŸ” Lagi search Spotify (Jere)...');
      try {
    ...
    if (cmd === 'jpin') {
      ...
      await interim(sock, jid, m, 'ðŸ“Œ Lagi search Pinterest (Jere)...');
      try {
        const pins = await jereUtil.jerePinSearch(args, 5);
        const img = jereUtil.pickFirstImageUrl ? jereUtil.pickFirstImageUrl(pins) : '';
        if (img) {
          await sock.sendMessage(jid, { image: { url: img }, caption: `ðŸ“Œ Pinterest: ${args.slice(0, 150)}` }, { quoted: m });
          return true;
        }
        await sendLongText(sock, jid, `ðŸ“Œ *Pinterest: ${args}*\n\n${JSON.stringify(pins.slice(0, 3), null, 2).slice(0, 3000)}`, m); return true;
      } catch (e) {
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, jereErr(e), m); return true;
      }
    ...
    if (cmd === 'jwallpaper') {
      ...
      await interim(sock, jid, m, 'ðŸ–¼ï¸ Lagi cari wallpaper (Jere)...');
      try {
        ...
        await sock.sendMessage(jid, { image: { url: u }, caption: `ðŸ–¼ï¸ Wallpaper: ${args.slice(0, 150)}` }, { quoted: m });
      } catch (e) {
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, jereErr(e), m); return true;
      }
    ...
    if (cmd === 'jcuaca') {
      ...
      await interim(sock, jid, m, 'ðŸŒ¤ï¸ Lagi cek cuaca (Jere)...');
      try {
    ...
    if (cmd === 'jbmkg') {
      await interim(sock, jid, m, 'ðŸŒ¤ï¸ Lagi cek BMKG (Jere)...');
      try {
    ...
    if (cmd === 'jlibur') {
      await interim(sock, jid, m, 'ðŸ—“ï¸ Lagi cek hari libur (Jere)...');
      try {
    ...
    if (cmd === 'jstyle') {
      ...
      try {
        ...
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, `âŒ ${String(e?.message || 'Gagal style teks.').slice(0, 200)}`, m); return true;
      }
    }
    if (cmd === 'jshort') {
      if (!args) { await safeReply(sock, jid, `Contoh: ${prefix}jshort https://example.com/panjang`, m); return true; }
      await interim(sock, jid, m, 'ðŸ”— Lagi perpendek link...');
      try {
        ...
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, `âŒ ${String(e?.message || 'Gagal perpendek link.').slice(0, 200)}`, m); return true;
      }
    }
    if (cmd === 'jnews') {
      ...
      await interim(sock, jid, m, 'ðŸ“° Lagi ambil berita (Jere)...');
      try {
    ...
    if (cmd === 'jgenius') {
      ...
      await interim(sock, jid, m, 'ðŸŽµ Lagi cari di Genius (Jere)...');
      try {
    ...
    if (cmd === 'jmlbb') {
      ...
      await interim(sock, jid, m, 'âš”ï¸ Lagi ambil build MLBB (Jere)...');
      try {
    ...
    if (cmd === 'jmlbbtier') {
      await interim(sock, jid, m, 'âš”ï¸ Lagi ambil tier MLBB (Jere)...');
      try {
        const tier = await jereUtil.jereMlbbTier();
        const top = (Array.isArray(tier) ? tier : []).slice(0, 15).map((h, i) => `${i + 1}. ${h.hero_name || h.hero || h.name || '-'} â€” Tier ${h.tier || '-'}`).join('\n');
        await safeReply(sock, jid, `âš”ï¸ *MLBB Tier (top 15)*\n\n${top || '-'}`, m); return true;
      } catch (e) {
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, jereErr(e), m); return true;
      }
    ...
    if (cmd === 'jff') {
      ...
      await interim(sock, jid, m, 'ðŸŽ® Lagi stalk FF (Jere)...');
      try {
    ...
    if (cmd === 'jspeed' || cmd === 'jping') {
      try {
        const cap = jereUtil.buildSpeedCaption ? jereUtil.buildSpeedCaption(Date.now() - start) : `ðŸ“ Speed: ${Date.now() - start}ms`;
        await safeReply(sock, jid, cap, m); return true;
      } catch (e) {
        await safeReply(sock, jid, `ðŸ“ Speed: ${Date.now() - start}ms`, m); return true;
      }
    }
    if (cmd === 'jos') {
      try {
        const info = jereUtil.collectOsInfo ? jereUtil.collectOsInfo() : null;
        const cap = jereUtil.formatOsCaption ? jereUtil.formatOsCaption(info) : 'ðŸ’» Info server tidak tersedia.';
        await sendLongText(sock, jid, String(cap).slice(0, 3500), m); return true;
      } catch (e) {
        await safeReply(sock, jid, 'âŒ Gagal ambil info server.', m); return true;
      }
    }
    // Owner aman saja (eval/exec/restart TIDAK di-wiring; spam TIDAK ada di lib).
    if (cmd === 'jbackup') {
      if (!isOwner(sender, jid)) { await safeReply(sock, jid, 'ðŸ”’ Khusus owner ya.', m); return true; }
      try {
        ...
        for (const f of files.slice(0, 20)) {
          try {
            ...
            totalBytes += st.size;
            rows.push(`â€¢ ${f} (${(st.size / 1024).toFixed(1)} KB)`);
          } catch {}
        }
        const name = jereUtil.buildBackupDbFileName ? jereUtil.buildBackupDbFileName('database') : `database-${Date.now()}.json`;
        await safeReply(sock, jid, `ðŸ’¾ *Backup DB (ringkasan)*\nðŸ“„ Nama: ${name}\nðŸ“Š Total: ${(totalBytes / 1024).toFixed(1)} KB\n${rows.join('\n') || '(database kosong)'}`, m); return true;
      } catch (e) {
        await safeReply(sock, jid, `âŒ ${String(e?.message || 'Gagal backup.').slice(0, 200)}`, m); return true;
      }
    }
    if (cmd === 'jplugins') {
      if (!isOwner(sender, jid)) { await safeReply(sock, jid, 'ðŸ”’ Khusus owner ya.', m); return true; }
      try {
        ...
        await safeReply(sock, jid, `âŒ ${String(e?.message || 'Gagal baca plugin.').slice(0, 200)}`, m); return true;
      }
    }
    if (cmd === 'jjoin') {
      if (!isOwner(sender, jid)) { await safeReply(sock, jid, 'ðŸ”’ Khusus owner ya.', m); return true; }
      ...
      try {
        ...
        await safeReply(sock, jid, 'âœ… Bot berhasil join grup!', m); return true;
      } catch (e) {
        await safeReply(sock, jid, `âŒ Gagal join: ${String(e?.message || 'link salah/kadaluarsa.').slice(0, 200)}`, m); return true;
      }
    }
