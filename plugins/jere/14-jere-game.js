// handlers/jere-game.js â€” JERE-GAME/FUN (jawab tetap di games.js spt asli)
...
async function handleJereGame(ctx) {
  const { sock, m, jid, isGroup, sender, body, cmd, args, prefix, start, unwrapped } = ctx;
    ...
    if (cmd === 'jkuis' || cmd === 'jquiz') {
      ...
      await interim(sock, jid, m, 'â“ Lagi ambil soal (Jere)...');
      try {
        ...
        let txt = `â“ *Kuis Jere [${soal.game}]*\n${soal.soal}`;
        if (soal.clue) txt += `\nðŸ’¡ Clue: ${soal.clue}`;
        txt += `\n\nJawab pakai ${prefix}jawab <teks> (atau ${prefix}jawab nyerah) â€¢ 60 dtk`;
    ...
    if (cmd === 'animequotes' || cmd === 'janimeq') {
      try {
        ...
        let txt = `ðŸ’¬ *Anime Quotes*\n\n"${r.quote || '-'}"\n\nðŸ‘¤ ${r.character || '-'} â€¢ ðŸŽ¬ ${r.anime || '-'}`;
        if (r.image) {
          try {
    ...
    if (cmd === 'fakta' || cmd === 'jfact') {
      try {
        ...
        await safeReply(sock, jid, `ðŸ§  *Fakta Unik:*\n${r.fakta}`, m); return true;
      } catch (e) {
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, jereErr(e), m); return true;
      }
    ...
    if (cmd === 'alkitab') {
      await interim(sock, jid, m, 'ðŸ“– Lagi buka Alkitab (Jere)...');
      try {
        const r = await jereFun.alkitab(args || '');
        const out = typeof r === 'string' ? r : JSON.stringify(r, null, 2);
        await sendLongText(sock, jid, `ðŸ“– *Alkitab ${args || 'â€”'}*\n\n${String(out).slice(0, 3500)}`, m); return true;
      } catch (e) {
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, jereErr(e), m); return true;
      }
    ...
    if (cmd === 'tukar') {
      try {
        ...
        if (calc.needInput) { await safeReply(sock, jid, `Contoh: ${prefix}tukar 5 / ${prefix}tukar all\nKurs: ${calc.rate} koin = 1 limit â€¢ Koin kamu: ${calc.coin}`, m); return true; }
        await safeReply(sock, jid, `ðŸ’± *Tukar Koin*\n${calc.count} limit â†” ${calc.cost} koin (kurs ${calc.rate}/limit)\nSisa koin: ${calc.sisa}\n\nCatatan: penambahan limit dilakukan manual oleh sistem economy yang aktif.`, m); return true;
      } catch (e) {
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, `âŒ ${String(e?.message || 'Gagal tukar koin.').slice(0, 300)}`, m); return true;
      }
    }
