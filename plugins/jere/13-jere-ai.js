...
async function handleJereAi(ctx) {
  const { sock, m, jid, isGroup, sender, body, cmd, args, prefix, start, unwrapped } = ctx;
    ...
    if (cmd === 'jtxt2img' || cmd === 'jimg') {
      ...
      await interim(sock, jid, m, 'ðŸŽ¨ Lagi generate gambar (Jere)...');
      try {
        const buf = await jereAi.txt2img(args);
        await sock.sendMessage(jid, { image: buf, caption: `ðŸŽ¨ ${args.slice(0, 200)}` }, { quoted: m });
      } catch (e) {
    ...
    if (cmd === 'jtxt2vid' || cmd === 'jvideo') {
      ...
      await interim(sock, jid, m, 'ðŸŽ¬ Lagi generate video (Jere, bisa 1-3 menit)...');
      try {
        const buf = await jereAi.txt2video(args);
        await sock.sendMessage(jid, { video: buf, mimetype: 'video/mp4', caption: `ðŸŽ¬ ${args.slice(0, 200)}` }, { quoted: m });
      } catch (e) {
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, jereErr(e), m); return true;
      }
    ...
    if (cmd === 'jsora') {
      ...
      await interim(sock, jid, m, 'ðŸŽ¬ Lagi generate video Sora (Jere, bisa lama)...');
      try {
    ...
    if (cmd === 'jsuno' || cmd === 'jlagu') {
      ...
      await interim(sock, jid, m, 'ðŸŽµ Lagi bikin lagu (Jere, bisa 2-4 menit)...');
      try {
    ...
    if (cmd === 'jchat') {
      ...
      await interim(sock, jid, m, 'ðŸ’¬ Lagi mikir (Jere)...');
      try {
    ...
    if (cmd === 'jimg2vid') {
      ...
      await interim(sock, jid, m, 'ðŸŽ¬ Lagi animasikan gambar (Jere, bisa lama)...');
      try {
        ...
        await sock.sendMessage(jid, { video: out, mimetype: 'video/mp4', caption: 'ðŸŽ¬ img2video (Jere)' }, { quoted: m });
      } catch (e) {
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, jereErr(e), m); return true;
      }
    ...
    if (cmd === 'jupscale' || cmd === 'jhd') {
      ...
      await interim(sock, jid, m, 'âœ¨ Lagi upscale gambar (Jere)...');
      try {
        const target = quotedImg ? wrapQuoted(jid, quoted) : m;
        const buf = await downloadBuffer(target, sock);
        const out = await jereAi.upscale(buf);
        await sock.sendMessage(jid, { image: out, caption: 'âœ¨ upscale (Jere)' }, { quoted: m });
      } catch (e) {
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, jereErr(e), m); return true;
      }
    ...
    if (cmd === 'jtoanime') {
      ...
      await interim(sock, jid, m, 'ðŸŽ¨ Lagi ubah jadi anime (Jere)...');
      try {
        const target = quotedImg ? wrapQuoted(jid, quoted) : m;
        const buf = await downloadBuffer(target, sock);
        const out = await jereAi.toanime(buf, args || 'Japanese Anime');
        await sock.sendMessage(jid, { image: out, caption: 'ðŸŽ¨ toanime (Jere)' }, { quoted: m });
      } catch (e) {
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, jereErr(e), m); return true;
      }
    ...
    if (cmd === 'jclone') {
      ...
      await interim(sock, jid, m, 'ðŸŽ™ï¸ Lagi clone suara (Jere)...');
      try {
    ...
    if (cmd === 'jswap') {
      ...
      await interim(sock, jid, m, 'ðŸ”„ Lagi faceswap (Jere)...');
      try {
        ...
        await sock.sendMessage(jid, { image: out, caption: 'ðŸ”„ faceswap (Jere)' }, { quoted: m });
      } catch (e) {
        console.error(cmd, e?.message || e);
        await safeReply(sock, jid, jereErr(e), m); return true;
      }
