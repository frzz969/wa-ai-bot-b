...
async function postForm(path, form, query = {}, timeoutMs = 120000) {
  ...
  // WAJIB 'apikey' (bukan 'key') — konsisten dengan jereGet di lib/jere/jere-api.js.
  ...
  if (ct.includes('image/') || ct.includes('video/') || ct.includes('audio/') || ct.includes('application/octet-stream')) {
    if (!res.ok) throw new Error(`Server gagal: HTTP ${res.status}`);
    const buf = Buffer.from(await res.arrayBuffer());
  ...
  if (!res.ok) {
    throw new Error(`Server gagal: ${(data && (data.error || data.message)) || `HTTP ${res.status}`}`);
  }
