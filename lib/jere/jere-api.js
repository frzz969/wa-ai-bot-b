...
function getKey() {
  ...
  if (!key) {
    throw new Error('Fitur ini butuh key: isi JERE_API_KEY di .env');
  }
  return key;
}
...
async function jereGet(path, params = {}, timeoutMs = 60000) {
  ...
  // WAJIB 'apikey' (bukan 'key'). Server menolak 'key' dengan HTTP 401
  // "Login diperlukan" walau key-nya valid. Lihat juga komentar pola URL di lib/jere/*.
  ...
  try {
    ...
    if (ct.includes('image/') || ct.includes('application/octet-stream')) {
      if (!res.ok) throw Object.assign(new Error(`Server gagal: HTTP ${res.status}`), { code: 'JERE_API_HTTP', status: res.status });
    ...
    if (!res.ok) {
      ...
      const err = new Error(`Server gagal: ${detail}`);
...
async function jereTiktok(url) {
  ...
  if (!no_watermark) throw new Error('Server tidak mengembalikan link video TikTok.');
...
async function jereIqc(text, opts = {}) {
  ...
  if (Buffer.isBuffer(result)) {
    if (!result.length) throw new Error('Server mengembalikan gambar kosong.');
    return result;
  }
  ...
  if (typeof link === 'string' && /^https?:\/\//i.test(link)) {
    ...
    if (!res.ok) throw new Error(`Server gagal mengambil gambar (HTTP ${res.status}).`);
    const buf = Buffer.from(await res.arrayBuffer());
    if (!buf.length) throw new Error('Server mengembalikan gambar kosong.');
    return buf;
  }
