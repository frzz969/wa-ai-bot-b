...
// -------------------------------------------------------------- emojitopng ---
// Emoji -> PNG transparan 512px via SVG + sharp.
// Catatan: hasil glyph tergantung font emoji yang tersedia di OS/container
// (librsvg pakai font sistem). Tetap lokal, tanpa API.
// Contoh:
...
async function emojiToPng(emoji, size) {
  ...
  const e = String(emoji || '🙂').trim().split(/\s+/)[0].slice(0, 8);
  const svg =
    `<svg width="${s}" height="${s}" xmlns="http://www.w3.org/2000/svg">` +
    `<text x="${s / 2}" y="${s / 2}" text-anchor="middle" dominant-baseline="central" ` +
    `font-size="${Math.floor(s * 0.72)}">${escapeXml(e)}</text></svg>`;
  return await sharp(Buffer.from(svg)).png().toBuffer();
}
