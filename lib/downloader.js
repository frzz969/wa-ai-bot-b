...
const FRIENDLY_DOWNLOAD_ERROR = 'Maaf, fitur downloader sedang gagal diproses. Silakan coba lagi beberapa saat lagi. Fitur ini sedang dalam perbaikan. Hubungi admin jika masih gagal.';
...
async function ensureTools(needFfmpeg) {
  ...
  if (!hasYtDlp) {
    const error = new Error('yt-dlp unavailable');
...
async function ytSearch(query) {
  ...
  if (!yts) {
    const error = new Error('yt-search unavailable');
    error.code = 'DEPENDENCY_UNAVAILABLE';
    throw error;
  }
...
async function handlePlay(sock, jid, m, args) {
  ...
  try {
    ...
    await sock.sendMessage(jid, { text: FRIENDLY_DOWNLOAD_ERROR }, { quoted: m });
  } finally {
